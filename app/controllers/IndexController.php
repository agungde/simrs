...
class IndexController extends BaseController{
	...
	function register($formdata = null){
		if($formdata){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename;
			$fields = $this->fields = array("nama","username","email","password","photo"); //registration fields
...
$koneksi        = new mysqli($dbhost, $dbuser,$dbpass, $dbname);
 $query = mysqli_query($koneksi, "SELECT * from user_login WHERE id_userlogin='$rec_id'")
                                  or die('Ada kesalahan pada query tampil data : ' . mysqli_error($koneksi));
  $rows = mysqli_num_rows($query);
  if ($rows <> 0) {
    $data    = mysqli_fetch_assoc($query);
    $cekrole = trim($data['user_role_id']);
    if ($cekrole=="0" or $cekrole==""){
   $db->rawQuery("UPDATE user_login SET user_role_id='2' WHERE id_userlogin='$rec_id'");
    }else {}
}
		...
		if(!empty($email)){
			...
			if(!empty($user)){
				if(strtolower($user['email']) != 'verified'){
					$hashvalue = hash_value($user['id_userlogin']);
					$verify_link = SITE_ADDR."index/verifyemail/" . md5($user['id_userlogin']) . "?h=$hashvalue";
	...
	function verifyemail($user_id = null){
		if(!empty($user_id)){
			$db = $this->GetModel();
			$tablename = $this->tablename;
			$db->where ("md5(id_userlogin)", $user_id);//get user by userid hash value
			$user = $db->getOne($tablename);
			$hashed_user_id = $this->request->h; // get User Hashed Value
			if(!empty($user)){
				$server_hash_id = hash_value($user['id_userlogin']);
